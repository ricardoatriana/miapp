...
const MyOrders = ({ userId }) => { // <- receive the userId as a prop
  ...
  const handleDelete = async (orderId) => {
    try {
      const response = await axios.delete(`http://localhost:9000/api/orders/${orderId}`, { withCredentials: true });
      console.log(response.data.message); // Handle success message
      setOrders(orders.filter(order => order._id !== orderId)); // Remove order from the state
    } catch (err) {
